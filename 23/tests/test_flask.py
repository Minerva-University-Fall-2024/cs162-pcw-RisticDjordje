import unittest
from flask_app.server import app, db, User, Expression
...
class FlaskAppTests(unittest.TestCase):
    ...
    def test_register(self):
        ...
        response = self.app.post(
            "/register",
            data=dict(email="test@example.com", password="securepassword"),
    ...
    def test_login_logout(self):
        ...
        response = self.app.post(
            "/login",
            data=dict(email="user@example.com", password="mypassword"),
            follow_redirects=True,
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_dashboard_access(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            b"<!-- HTML to display the dashboard and history -->", response.data  # noqa
        )

    def test_expression_evaluation(self):
        response = self.app.post(
            "/evaluate", data=dict(expression="2+2"), follow_redirects=True
        )
        self.assertIn(b"<!-- HTML form for login -->", response.data)
